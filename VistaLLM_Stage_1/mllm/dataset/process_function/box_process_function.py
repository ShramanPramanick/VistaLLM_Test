...
from ..utils.transform import norm_box_xyxy, norm_point_xyxy, norm_mask_xyxy
    ...
    def __call__(self, raw_conv: List[Dict[str, Any]], target: Dict[str, Any], preprocessor: Dict[str, Any],
            ...
            if target is not None and 'masks' in target:
                for mask in target['masks']:
                    normalized_masks.append(
                        norm_mask_xyxy(mask, w=target['width'], h=target['height'])
                    )
        ...
        for sentence in raw_conv:
            ...
            if points_seq is not None:
                ...
                words = converted
            masks_seq: List[List[int]] = sentence.get('masks_seq', None)
            if masks_seq is not None:
                # map point seq
                masks_seq: List[Boxes] = map_obj(normalized_masks, masks_seq)
                # reformat; replace <points> placeholder
                converted = box_formatter.call_on_mask(words, masks_seq)
                words = converted
...
class BoxFormatter:
    ...
    def call_on_mask(self, sentence: str, masks_seq: BoxesSeq) -> str:
        all_box = self.masks_token_pat.findall(sentence)
        assert len(all_box) == len(masks_seq), f"not match. sentence: {sentence}. boxes:{masks_seq}"
        if len(all_box) == 0:
            return sentence
        bboxes_strs = [self.format_mask(bboxes) for bboxes in masks_seq]
        converted = sentence.replace(self.masks_token, '{}').format(*bboxes_strs)
        return converted
    ...
    def format_mask(self, masks) -> str:
        raise NotImplementedError
    ...
    def extract_mask(self, string: str) -> List[Boxes]:
        raise NotImplementedError
...
class TokenFormatter(BoxFormatter):
    ...
    def __init__(self, num_bins=1001):
        ...
        self.extract_mask_pat = re.compile(r'<m_st><bin_\d*?>(?:<bin_\d*?>){'+ str(2*MASKS_POINTS-1) +'}(?:<m_sep><bin_\d*?>(?:<bin_\d*?>){'+ str(2*MASKS_POINTS-1) +'})*<m_ed>')
    ...
    def format_mask(self, masks) -> str:
        ...
        for bbox in masks:
            ...
            for idx in range(0, len(bbox)-1, 2):
                current_x = "<bin_{}>".format(round((bbox[idx] * (self.num_bins - 1))))
                current_y = "<bin_{}>".format(round((bbox[idx+1] * (self.num_bins - 1))))
                region_coord += str(current_x) + " " + str(current_y) + " "
            final_str.append(region_coord[:-1])
        if self.use_sep:
            final_str = self.mask_sep.join(final_str)
        else:
            final_str = ''.join(final_str)
        if self.use_begin_end:
            final_str = self.mask_begin + final_str + self.mask_end
        return final_str
    ...
    def extract_mask(self, string: str) -> List[Boxes]:
        ret = []
        for bboxes_str in self.extract_mask_pat.findall(string.replace(" ", "")): ### replace not sure
            bboxes = []
            bbox_strs = bboxes_str.replace(self.mask_begin, "").replace(self.mask_end, "").split(self.mask_sep)
            for bbox_str in bbox_strs:
                elems = list(map(int, re.findall(r'<bin_(\d*?)>', bbox_str)))
                bbox = [elem / (self.num_bins - 1) for elem in elems]
                bboxes.append(bbox)
    ...
    def extract_point(self, string: str) -> List[Boxes]:
        ret = []
        for bboxes_str in self.extract_point_pat.findall(string.replace(" ", "")):
            bboxes = []
    ...
    def post_process_model_tokenizer(self, model, preprocessor, model_args, training_args):
        ...
        additional_special_tokens = [
            ...
            self.mask_begin, self.mask_sep, self.mask_end
        ]
