...
def norm_mask_xyxy(mask, *, w, h):
    for idx, coord in enumerate(mask):
        if idx%2 == 0:
            coord = max(0.0, min(coord / w, 1.0))
        else:
            coord = max(0.0, min(coord / h, 1.0))
        mask[idx] = coord
    return mask
...
def mask_xyxy_expand2square(mask, *, w, h):
    if w == h:
        return mask
    if w > h:
        for idx in range(1, len(mask), 2):
            mask[idx] += (w - h) // 2
        return mask
    assert w < h
    for idx in range(0, len(mask)-1, 2):
        mask[idx] += (h - w) // 2
    return mask
...
class Expand2square:
    ...
    def __call__(self, image: Image.Image, labels: Dict[str, Any] = None) -> Tuple[Image.Image, Optional[Dict[str, Any]]]:
        ...
        if 'masks' in labels:
            masks = [mask_xyxy_expand2square(mask, w=width, h=height) for mask in labels['masks']]
            labels['masks'] = masks
        return processed_image, labels
