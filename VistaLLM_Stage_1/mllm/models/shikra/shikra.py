...
# class Blip2BaseSupport(Blip2Base):

#     def __init__(self, num_query_token=32, num_clip_features=1024, vit_model="eva_clip_g", img_size=224, 
#                  drop_path_rate=0, use_grad_checkpoint=True, vit_precision="fp16"):
...
class ShikraLlamaModel(LlamaModel):
    ...
    def initialize_vision_modules(self, vision_tower, mm_vision_select_layer,
        ...
        eva_model, _, image_processor = create_model_and_transforms(vision_tower, pretrained="eva_clip", force_custom_clip=True)

        if not hasattr(self, 'vision_tower'):
            # vision_tower = CLIPVisionModel.from_pretrained(vision_tower)
            vision_tower = eva_model
        else:
        ...
        vision_config = vision_tower.config = ClipConfig(image_size=224, patch_size=14, hidden_size=1408)
        ...
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            with torch.no_grad():
                if type(images) is list:
                    ...
                    image_forward_outs = vision_tower.visual(images, return_all_features=True)
                    select_hidden_state_layer = getattr(self.config, "mm_vision_select_layer", -1)
            ...
            dummy_image_features = torch.zeros(256, self.config.mm_hidden_size, device=inputs_embeds.device, dtype=inputs_embeds.dtype)
...
class ShikraLlamaForCausalLM(LlamaForCausalLM):
    ...
    def initialize_vision_tokenizer(self, mm_use_im_start_end, tokenizer, device,
        ...
        vision_config = self.model.vision_tower[0].config = ClipConfig(image_size=224, patch_size=14, hidden_size=1408)
